...
// 9 seções (pedido explícito desta tarefa — eram 6; ganhou "palavras",
// "numeros" e "cursos"), NA ORDEM em que aparecem na página. "palavras" e
// "numeros" não têm item de menu próprio (ver ITENS_MENU em
// LandingHeader.tsx — 7 itens, um pra cada UMA das outras 7 seções) — só
// entram no scroll spy porque fazem parte do fluxo normal de scroll, mas
// ninguém precisa "pular" direto pra elas pelo menu.
const SECOES = [
  'inicio',
  'palavras',
  'numeros',
 ...
 * (single-page), com o header fixo (LandingHeader.tsx) navegando por
 * scroll suave entre seções âncora, em vez de rotas separadas.
 *
 ...
 *
 * scroll-mt-16 em cada seção: compensa a altura do header FIXO (h-16
 * "cheio"/topo da página — ver LandingHeader.tsx; ao rolar ele encolhe
 * pra h-12, mas a folga de 16 continua servindo de sobra, nunca deixando
 * nada coberto) — sem isso, o topo da seção ficaria escondido atrás do
 * header ao navegar (scrollIntoView alinha o topo da seção exatamente no
 * topo da viewport, que é onde o header fixo está).
 *
 * `destinoLogado`: null pra visitante deslogado (header mostra
 * "Entrar"/"Criar conta grátis"); '/inicio' ou '/admin/dashboard' quando
 * já existe sessão — troca os dois botões por um único "Ir para a
 * plataforma" apontando pra lá. Decidido no servidor (app/page.tsx, já tem
 * o profile ali) e só repassado pra cá/pro header — nenhuma chamada extra
 * ao Supabase no client só pra saber isso.
 *
 * `numeroWhatsapp`: mesma configuração (`configuracoes.numero_whatsapp`)
 * já usada em todo o resto da plataforma — só repassada pro botão/pop-up
 * de suporte flutuante (LandingFloatingActions.tsx).
 */
export default function LandingPageClient({
  ...
  return (
    <div className="bg-white text-gray-900 dark:bg-[#0a0a0a] dark:text-white">
      ...
      {/* pt-16: mesma altura do header fixo "cheio" (h-16) — sem isso a
          primeira seção (Início) nasceria coberta por ele. */}
      <main className="pt-16">
        {SECOES.map((id) => (
          <section
            ...
            className="flex min-h-screen scroll-mt-16 flex-col items-center justify-center border-b border-gray-200 px-6 text-center dark:border-white/10"
          >
            {/* Placeholder (pedido explícito) — só o nome da seção, pra
                validar a navegação/scroll-spy visualmente. Substituir pelo
                conteúdo de verdade de cada seção é tarefa futura. */}
