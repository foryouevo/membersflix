          ...
          {modulos.map((modulo, mIndex) => (
            <div key={modulo.id} className="overflow-hidden rounded-lg bg-card">
              <div className="flex items-center gap-2 px-4 py-3">
                ...
                <button
                  onClick={() => setOpenModuloId(openModuloId === modulo.id ? null : modulo.id)}
                  className="flex flex-1 items-center justify-between text-left"
                >
                  <span className="font-semibold text-white">{modulo.titulo}</span>
                  {openModuloId === modulo.id ? <ChevronUp size={18} /> : <ChevronDown size={18} />}
                </button>
                <div className="flex items-center gap-1">
                  <button disabled={isPending} onClick={() => moveModulo(mIndex, -1)} className="text-on-variant hover:text-white">
                    <ChevronUp size={16} />
                  </button>
                  <button disabled={isPending} onClick={() => moveModulo(mIndex, 1)} className="text-on-variant hover:text-white">
                    <ChevronDown size={16} />
                  ...
                  <button
                    onClick={() => {
                      if (confirm(`Excluir o módulo "${modulo.titulo}" e todas as suas aulas?`))
                        run(() => deletarModulo(modulo.id, curso.id));
                    }}
              ...
              {openModuloId === modulo.id && (
                <div className="border-t border-border/40">
                  {modulo.aulas.map((aula, aIndex) => (
                    <button
                      ...
                      className={`flex w-full items-center gap-3 border-b border-border/30 px-4 py-2.5 text-left last:border-0 hover:bg-surface-container ${
                        selectedAula?.id === aula.id ? 'bg-surface-high' : ''
                      }`}
                    >
                      <GripVertical size={14} className="text-on-variant" />
                      <div className="flex-1">
                        <p className="text-sm text-white">
                          {mIndex + 1}.{aIndex + 1} {aula.titulo}
                        </p>
                  ...
                  <button
                    onClick={() => handleAddAula(modulo.id, modulo.aulas.length)}
                    className="flex w-full items-center gap-2 px-4 py-3 text-sm text-primary hover:bg-surface-container"
                  >
